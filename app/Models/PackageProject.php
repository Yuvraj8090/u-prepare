...
class PackageProject extends Model
{
    ...
    protected $fillable = ['project_id', 'package_category_id', 'package_sub_category_id', 'department_id', 'package_component_id', 'package_name', 'package_number', 'estimated_budget_incl_gst', 'vidhan_sabha_id', 'lok_sabha_id', 'district_id', 'block_id', 'dec_approved', 'dec_approval_date', 'dec_letter_number', 'dec_document_path', 'hpc_approved', 'hpc_approval_date', 'hpc_letter_number', 'hpc_document_path', 'status'];
    ...
    */
    ...
    {
        return $query->select('id', 'package_name', 'status'); // ✅ include status in basic info
    }
    ...
    public function scopeApplyFilters($query, array $filters)
    {
        return $query
            ...
            ->when($filters['status'] ?? null, fn($q, $v) => $q->where('status', $v)) // ✅ filter by status
            ->when($filters['search'] ?? null, function ($q, $search) {
                $q->where(fn($sub) => $sub->where('package_name', 'like', "%{$search}%")->orWhere('package_number', 'like', "%{$search}%"));
            })
