...
<div class="container py-5">
    ...
    <div class="card shadow-lg border-0 mb-4">
        ...
        <div class="card-body">
            <x-admin.data-table 
                :headers="['Sub Project', 'Contract Value', 'Financial Progress', 'Physical Progress', 'Tests / Reports', 'Actions']" 
                id="progressTable" 
                :excel="true" 
                :print="true"
                :pageLength="10"
            >
                @foreach ($subProjects as $progress)
                    <tr class="align-middle">
                        ...
                        <!-- Financial Progress -->
                        <td>
                            <div title="{{ $progress->financial_progress_percentage }}% Financial" class="progress shadow-sm" style="height: 22px; border-radius: 12px;cursor: pointer;">
                                <div class="progress-bar bg-success progress-bar-striped progress-bar-animated fw-bold"
                                    ...
                                    aria-valuenow="{{ $progress->financial_progress_percentage }}" 
                        ...
                        <td>
                            @if ($progress->physical_progress_percentage > 0)
                                <div title="{{ $progress->physical_progress_percentage }}% Physical" class="progress shadow-sm" style="height: 22px; border-radius: 12px;cursor: pointer;">
...
<td>
    ...
    @if($totalTests > 0)
        ...
        @endif
        @if($pendingTests > 0)
            <span class="badge bg-warning text-dark ms-1">{{ $pendingTests }} Pending</span>
        @endif
    @else
